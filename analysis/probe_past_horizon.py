...
def main() -> None:
    ...
    print(f"{'range':>26s} {'probe MAE':>10s} {'constant MAE':>13s} "
          f"{'ratio':>6s} {'R2':>7s}")
    for key, lab in (("below", "k=2..32   (below N*)"),
                     ("past", "k=48..128 (past N*)")):
        v = rows[key]
    ...
    if not rows["past"]["beats_constant"]:
        print("Past the horizon the trained probe matches a predictor that ignores\n"
              "the states and always answers the mean, so the low R^2 is not an\n"
              "artifact of the narrower k range: it has learned nothing.\n"

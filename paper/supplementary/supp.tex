...
\section{The non-autoregressive baseline}
\label{sec:s13}

The paper's scope claim is about autoregressive decoders, and round-6 reviewers
observed that we had never run one that was not. VITS
(\texttt{facebook/mms-tts-eng}) supplies the contrast: a text encoder, a duration
predictor and a normalising-flow decoder that emits the waveform in a single
pass. There is no generation-step recurrence, so there is no per-repetition state
map for Assumption~1 to describe and Theorem~1 makes no prediction about it. If
the deficit appeared here too, the theorem would be the wrong account of it.
...
\midrule
AR pooled & 18.2 & 94.3 & +76.1 \\
VITS (non-AR) & 65.6 & 58.9 & -6.7 \\
\bottomrule
\end{tabular}
\end{center}

VITS shows no periodicity-specific deficit: its repeated items come back
\emph{more} often exact than its controls, a gap of -6.7 points against the
panel's +76.1, and every one of the 6 AR checkpoints separates from it.
It is not that VITS is immune to the stimuli --- it degrades with $k$ on both
families alike:

\begin{center}
\begin{tabular}{rrr}
\toprule
$k$ & repeated exact (\%) & control exact (\%) \\
\midrule
6 & 73.3 & 86.7 \\
8 & 80.0 & 46.7 \\
12 & 73.3 & 60.0 \\
...
\noindent\textbf{What this rules out, and what it does not.} It rules out the
reading on which the deficit is a property of long repeated \emph{text} --- the
same strings, through a decoder without recurrence, do not produce it. It does
not identify \emph{which} feature of autoregressive decoding is responsible;
VITS differs from the panel in more than its recurrence, having a duration
predictor whose output length is a sum of per-phoneme durations rather than a
learned stop decision, and being a smaller and older model. Absolute accuracy is
therefore not comparable between them and we do not compare it. The comparison is
the within-model dissociation, which is scale-free.

\noindent\textbf{A pacing check, which cuts our way.} One could object that VITS
avoids the deficit by rendering repeated words so slowly that the recogniser
cannot miss them. The opposite is true. Removing the carrier baseline, VITS
spends $0.305$\,s per requested unit on repeated items against $0.384$\,s on
controls --- it \emph{compresses} repetitions to $79\%$ of the control pace,
which is exactly the acoustic condition that makes CTC blank-collapse most
likely. Its repeated items are therefore the harder case for the judge, and they
are still counted correctly more often than the panel's.

\noindent\textbf{It is also the control the judge most needed.} Two rounds of
reviewers noted that CTC blank-collapse depresses counts on same-word runs
specifically (\S\ref{sec:s9-field}), which is the direction of our own effect,
and asked whether the recogniser manufactures the gap. Auditing the recogniser
against reference audio cannot settle this, because the audio it is audited on is
not the audio in question. The non-AR baseline can: it renders the same repeated
words, from the same stimulus file, transcribed by the same recogniser, and shows
no gap. Whatever blank-collapse costs us, it does not create the dissociation.


\subsection{Why the probe does not contradict Theorem~1(iv)}

A reviewer observed that a linear probe is exactly the $L$-Lipschitz readout
Theorem~1(iv) forbids past the horizon, so our recovering the count at
$R^2\approx0.48$ looks like a refutation of our own theorem. It is not, and the
reason is a scope limit worth stating rather than a defence.

The theorem forbids a Lipschitz readout from separating counts \emph{past}
$N^\ast$; below it, nothing is claimed. Our probe is fitted on states from the
main ladder, $k\le32$, and the fitted saturation scale for repeated items is
about $30$. The probe therefore operates entirely at or below the point where
tracking stops, in the regime where the theorem permits a readout to succeed.

This cuts both ways, which is why it belongs in the same paragraph as the
retraction above. The probe cannot refute the theorem, and it equally cannot
support it: the prohibition applies to a range we never probed. Probing states
from the extension ladder ($k=48\ldots128$), where the count has demonstrably
stopped tracking, is the experiment that would test it, and we did not run it ---
the extension was generated without the hidden-state instrumentation, which is a
straightforward thing for a follow-up to fix.

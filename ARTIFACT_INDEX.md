...
|---|---|---|
| Named-author source / readable PDF | `paper/main.tex`, `paper/build/main.pdf` | Current H9 working draft has four technical pages plus a references-only fifth page and contains authorized working metadata for the single-anonymous policy. Figure 1 displays the hash-validated H9 P/B1/B2 terminal comparison; see `paper/figures/FIGURE_H9_PCR_TERMINAL.md`. It is an internal draft; confirm portal metadata before upload. |
...
| Static local PDF preflight | `paper/SUBMISSION_READINESS.md`, `scripts/check_paper_pdf.py` | Current named-author H9 PDF passes five-page US-letter, Table 1 (page 2), References (page 5), and font checks in single-anonymous-submission mode. This is not IEEE PDF eXpress or an official template check. |
| Post-H10 paper-integrity check | `verification/POST_H10_PAPER_INTEGRITY_20260811.md` | Confirms that the H10 extension failed its own locked practical-effect gate, was retained/audited, and did not alter the H9-only readable manuscript. |
...
| Deadline completion record | `verification/DEADLINE_COMPLETION_20260810.md` | Final 20:00 UTC verification, explicit scientific stop boundaries, remote/worktree state, complete-history HDD-bundle identity, and redacted Telegram handoff receipt. |
| Citation ledger | `paper/citation-verification.md` | Current H9 citations are mapped to bounded claims and primary metadata checks. |
...
| Codex-only H9 paper review | `paper/reviews/h9_pcr_draft_20260811/` | Three independent Codex-only methods, narrative, and presentation audits plus a synthesis. This is internal technical review, not conference peer review. |
...
| H7 feature-only transfer audit | `experiments/h7_feature_transfer/protocol.md`, `experiments/h7_feature_transfer/results/h7_analysis_001/` | Complete fixed-recipe, score-free five-cell leave-one-corpus-out label-transfer matrix over all 28 features. It is descriptive only and cannot identify a detector cue, causal effect, or mitigation improvement. The sealed vector/raster display and retained layout iterations are documented in `H7_SUPPLEMENTARY_FIGURE_001.md`. |
| H8-SF robust score-fusion pivot | `experiments/h8_positive_pivot/PLAN.md` and `results/H8_SF_ANALYSIS_001.md` | Hash-sealed output under HDD `runs/h8_score_fusion/h8sf_target_evaluation_001/`. | Terminal failed candidate. GroupDRO beats B1/B2 but cannot beat the predeclared source-selected Spectra-AASIST B0 baseline (9.309% vs 0.185% mean EER). No H8 retuning or paper claim is authorized. |
| H9 same-item pair ranking | `experiments/h9_paired_counterfactual/PLAN.md`, `H9_LITERATURE_AUDIT_20260811.md` | `experiments/h9_paired_counterfactual/results/H9_TERMINAL_EVALUATION_001.md` | Complete positive controlled-transfer result. The locked four-seed P ensemble reaches 45.02% macro EER on SONAR+ArAD versus B1 51.78% and B2 52.01%; both 2,000-replicate shared-ID bootstrap CIs are below zero and all eight gate rules pass. The pairing key is documented metadata, not transcript-level semantics; it is not SOTA or a causal/general-mechanism claim, and H9 cannot be retuned. |
| H10 CD-ADD source-frozen follow-up | `experiments/future_directions/H10_CDADD_EXTERNAL_REPLICATION_PROTOCOL.md`, `H10_DECISION_RECORD_20260811.md` | `experiments/future_directions/results/H10_CDADD_TERMINAL_EVALUATION_001.md`, `reviews/H10_CDADD_TERMINAL_RED_TEAM_AUDIT_20260811.md` | Complete terminal no-retune extension. The H9 P panel is lower than B1/B2 on all-trial CD-ADD with both paired intervals below zero, but misses H10's locked 10% P-versus-B1 relative-EER rule (8.30%). The independent replay found no invalidating defect. It cannot extend the H9 manuscript claim or trigger a new target/model search. |
| H11 Counterpart Specificity Ladder | `experiments/future_directions/H11_IDEA_AUDIT_20260811.md`, `H11_TARGET_PANEL_AUDIT_20260811.md` | No result artifact exists by design. | Prospective relation-specificity replication: same-item versus same-voice/different-item, random-pair, and BCE with fresh AASIST/RawNet2. HABLA is conditional; J-SPAW_LA additionally requires explicit non-commercial-use authority. No H11 data, model, score, target, or paper claim exists. |
| Codex-only H7 supplementary review | `paper/reviews/codex_h7_supplement_20260810/review.md` | Read-only internal claim, provenance, and visual-integrity audit. It accepts the H7 figure as supplementary-only; it is not external peer review. |
...
The most recent full suite is 149 passed. `verification/FINAL_VERIFICATION_20260810.md` records the current PDF hash, static preflight, full test suite, reviewed main-figure revision, H6/H7 supplementary displays, and complete-history bundle evidence; rerun the commands after any relevant source change.

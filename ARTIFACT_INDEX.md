...
| H9 paired-counterfactual ranking | `experiments/h9_paired_counterfactual/PLAN.md` | No data or result artifact exists yet by design. | Independent active positive-result candidate. ODSS source pairing, source-only BF16 fitting, and the fixed SONAR+ArAD two-target evaluation are predeclared. A failed source-pairing/provenance/strict gate stops H9 rather than licensing a new loss, target, or source. |

...
from common.ras import (  # noqa: E402
    RAS_NOOP_THRESHOLD, RAS_THRESHOLD, RAS_WINDOW, RepetitionAwareSampling,
)
...
class TalkerGenerateCapture:
    ...
    `ras_factory` is the one thing this class does that is not read-only, and
    it is off unless `--ras` is passed. `Qwen3TTSForConditionalGeneration.
    generate()` builds its `talker_kwargs` dict literally and forwards nothing
    from `**kwargs` except two output flags, so a logits processor cannot reach
    the talker's sampling loop through the public API at all -- this call site
    is the only place it can be injected. The factory is handed the sampling
    kwargs the library actually chose (temperature/top_k/top_p), so the RAS
    nucleus branch is built from the checkpoint's real decoding configuration
    rather than from a copy of it that could drift.
    """

    def __init__(self, talker, ras_factory=None):
        self.talker = talker
        self.result = None
        ...
        def capturing_generate(*args, **kwargs):
            if ras_factory is not None:
                self.ras = ras_factory(kwargs)
                self.ras.reset()
                kwargs["logits_processor"] = LogitsProcessorList([self.ras])
...
def main() -> None:
    ...
    # Repetition Aware Sampling (VALL-E 2, arXiv:2406.05370). Off unless --ras
    # is passed, and when off not one line of this file's behaviour changes:
    # the existing arms were generated by the code path that runs when
    # `ras_factory is None`. See src/common/ras.py for the pre-commitment and
    # for every decision the method description leaves open.
    ap.add_argument("--ras", action="store_true",
                    help="repetition-aware sampling on the talker's codebook-0 stream")
    ap.add_argument("--ras-window", type=int, default=RAS_WINDOW,
                    help="K, the repetition window (paper: 10)")
    ap.add_argument("--ras-threshold", type=float, default=RAS_THRESHOLD,
                    help=f"t_r (paper: 0.1); {RAS_NOOP_THRESHOLD} is the no-op "
                         "setting, unreachable by construction")
    ap.add_argument("--ras-top-p", type=float, default=None,
                    help="top-p of the RAS nucleus branch; default: the "
                         "checkpoint's shipped value")
    ap.add_argument("--ras-top-k", type=int, default=None)
    ap.add_argument("--ras-temperature", type=float, default=None)
    ap.add_argument("--ras-fallback", choices=("full", "tempered"), default="full",
                    help="'random sampling' branch: the untruncated, untempered "
                         "model distribution (literal) or temperature-kept")
    ap.add_argument("--ras-no-self-term", action="store_true",
                    help="drop the k=0 term of the printed ratio (the other "
                         "reading; not used for the verdict)")
    args = ap.parse_args()
    check_gpu(args.gpu)
    ...
    ras_factory = None
    if args.ras:
        def ras_factory(gen_kwargs, _a=args):  # noqa: F811
            return RepetitionAwareSampling(
                window=_a.ras_window, threshold=_a.ras_threshold,
                temperature=(_a.ras_temperature if _a.ras_temperature is not None
                             else gen_kwargs.get("temperature")),
                top_k=(_a.ras_top_k if _a.ras_top_k is not None
                       else gen_kwargs.get("top_k")),
                top_p=(_a.ras_top_p if _a.ras_top_p is not None
                       else gen_kwargs.get("top_p")),
                fallback=_a.ras_fallback, self_term=not _a.ras_no_self_term)
        print(f"[{args.model}] RAS on: K={args.ras_window} t_r={args.ras_threshold} "
              f"fallback={args.ras_fallback} self_term={not args.ras_no_self_term} "
              f"nucleus(top_p={args.ras_top_p} top_k={args.ras_top_k} "
              f"temp={args.ras_temperature}, None=checkpoint default)", flush=True)
    capture = TalkerGenerateCapture(talker, ras_factory=ras_factory)
    ...
    for seed in args.seeds:
        for it in items:
            ...
            # `n_steps >= max_new_tokens` has never once fired -- see
            # `decode_step_count`'s docstring: the prefill call makes the step
            # count undercount the generated tokens by exactly one, so a
            # generation that saturates the budget records 2047 against 2048 and
            # reports `hit_cap=False`. The already-written ledgers were repaired
            # in place from `n_speech_tokens`; this line is the same repair
            # applied at write time, so new arms do not need repairing and
            # `population.panel()`'s cap-hit rule can actually see them. The old
            # (never-true) condition is kept in the disjunction so that nothing
            # the old rule would have caught is lost.
            hit_cap = bool(n_steps >= args.max_new_tokens
                           or n_speech_tokens >= args.max_new_tokens - 1)
            ...
            # How often the rule actually fired, per item. A mitigation that
            # never fires cannot be shown not to work, so this is the arm's
            # engagement gate and it is recorded per generation rather than
            # asserted once.
            if capture.ras is not None:
                rec.update(capture.ras.stats())
